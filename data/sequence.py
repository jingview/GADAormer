...
class SequenceDataset(Dataset):
    def __init__(self, seq_data, labels,pseudo_labels):
    ...
    def __getitem__(self, idx: torch.LongTensor):
        ...
        return batch_seq, batch_labels, idx
...
def split_dataset(data):
    seq_data, labels, train_nid, val_nid, test_nid,pseudo_labels, *infos = data
    dataset = SequenceDataset(seq_data, labels,pseudo_labels)
...
def load_sequence_data(args):
    ...
    pseudo_labels = torch.LongTensor(info_data['pseudo_labels'])
    ...
    original_train_nid = torch.LongTensor(info_data['original_train_nid'])
    ...
    print(f"[Global] Dataset <{args['dataset']}> Overview\n"
          ...
          f"\tTrain  (postive/total) {torch.sum(labels[original_train_nid]):>6} / {labels[train_nid].shape[0]:<6}\n"
    ...
    return seq_data, labels, train_nid, val_nid, test_nid, feat_dim, n_classes, n_relations,pseudo_labels,original_train_nid
